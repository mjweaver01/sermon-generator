...
export const handler = async (event: any, context: any) => {
    ...
    try {
      const pathsToTry = [
        ...
        process.env.LAMBDA_TASK_ROOT
          ? join(process.env.LAMBDA_TASK_ROOT, 'markdown-files')
          : null,
        process.env.LAMBDA_TASK_ROOT
          ? join(process.env.LAMBDA_TASK_ROOT)
          : null,
      ...
      for (const path of pathsToTry) {
        try {
          ...
          files = dirContents.filter(file => file.endsWith('.md')).sort()
  ...
  try {
    ...
    const pathsToTry = [
      ...
      // Alternative Netlify paths
      process.env.LAMBDA_TASK_ROOT
        ? join(process.env.LAMBDA_TASK_ROOT, 'markdown-files', fullFilename)
        : null,
      process.env.LAMBDA_TASK_ROOT
        ? join(process.env.LAMBDA_TASK_ROOT, fullFilename)
        : null,
    ].filter(Boolean) as string[]
    ...
    if (!found) {
      return {
        ...
        body: JSON.stringify({
          error: `Markdown file '${fullFilename}' not found`,
        }),
      }
    }
